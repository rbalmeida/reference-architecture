...
In the documentation provided [1] there are some examples of usage of Pure Agentic AI and one of them is for a sample Banking System with agents, so I've made some tests using this approach, full project is available.
...
When testing and invoking the agentic flow a few times, eventually some errors occurred when tool still mas not properly called. For example instead of using the exchange tool it would use a generated response.
...
The eventual non-deterministic behavior is an aspect that could cause issues in a production solution, so better understanding on how to control and configure the behavior would be important in those contexts.
...
2. Test project, available at [pure-agents-banking](/ai-studies/langchain4j-examples/pure-agents-banking)
